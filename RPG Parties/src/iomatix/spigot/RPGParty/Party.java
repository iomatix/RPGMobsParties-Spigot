...
public class Party implements IParty {
	...
	private ArrayList<String> membersInRange;
	...
	public Party(final Parties plugin, final Player leader) {
		...
		this.membersInRange = new ArrayList<String>();
	...
	public void checkInvitations() {
		...
		for (final String member : array) {
			if (this.invitations.get(member) < System.currentTimeMillis()) {
				...
				if (player != null) {
					this.sendMessages(this.plugin.getMessage("Party.no-response", true,
	...
	public void changeLeader() {
		for (final String member : this.members) {
			if (Server.isOnline(member)) {
				...
				this.sendMessages(this.plugin.getMessage("Party.new-leader", true,
						Filter.PLAYER.setReplacement(this.partyLeader.getName())));
			}
		}
	}
	...
	public int checkMembersInDistance(final Player source) {
		...
		for (final String member : this.members) {
			final Player player = Server.getPlayer(member);
			if (player != null) {
				if ((this.plugin.getMaxDistance() >= player.getLocation().distance(
						source.getLocation()) && player.getWorld().getName().equals(source.getWorld().getName()))
	...
	public boolean giveExp(final Player source, final double amount, final ExpSource expSource) {
		...
		for (final String member : this.membersInRange) {
			final Player player = Server.getPlayer(member);
			if (player != null) {
	...
	public void sendMessage(final Player sender, final String message) {
		this.sendMessages(this.plugin.getMessage("Party.chat-message", true,
				Filter.PLAYER.setReplacement(sender.getName()), Filter.MESSAGE.setReplacement(message)));
	}
