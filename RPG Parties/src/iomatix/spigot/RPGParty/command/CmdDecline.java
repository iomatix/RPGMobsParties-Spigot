...
public class CmdDecline implements IFunction
{
    public void execute(final ConfigurableCommand command, final Plugin plugin, final CommandSender sender, final String[] args) {
        ...
        if (party != null && party.isInvited(player)) {
            ...
            party.sendMessages(parties.getMessage("Party.player-declined", true, Filter.PLAYER.setReplacement(player.getName())));
            parties.sendMessage(player, "Individual.declined", new CustomFilter[0]);
        }
        else {
